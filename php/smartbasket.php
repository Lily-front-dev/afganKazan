	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (isset($_POST['city']) ) {
			if(empty($_POST['city'])) {
				...
				$city = "<b>Населенный пункт: </b>" . strip_tags($_POST['city']) . "<br> ";
			}
		}
		if (isset($_POST['delivery']) ) {
			if(empty($_POST['delivery'])) {
				...
				$delivery = "<b>Способ доставки: </b>" . strip_tags($_POST['delivery']) . "<br>";
			}
		}
		if (isset($_POST['region']) ) {
			if(empty($_POST['region'])) {
				...
				$region = "<b>Область: </b>" . strip_tags($_POST['region']) . "<br>";
			}
		}
		if (isset($_POST['payment']) ) {
			if(empty($_POST['payment'])) {
				...
				$payment = "<b>Способ оплаты: </b>" . strip_tags($_POST['payment']) . "<br>";
			}
		}
		if (isset($_POST['address']) ) {
			if(empty($_POST['address'])) {
				...
				$payment = "<b>Адрес доставки/Номер отделения: </b>" . strip_tags($_POST['address']) . "<br>";
			}
		}
		...
		$bodyHeader = '<table border="0" cellpadding="0" cellspacing="0" style="border-bottom:1px; border-right:1px; border-color:#e2e2e2; border-style: solid; width:800px" width="800" align="center">
			<tr >
				<th colspan="3" style="width: 400px; padding-top:15px; padding-bottom:15px; padding-right:15px; padding-left:15px; text-align:left; border-top:1px; border-left:1px; border-right:0; border-bottom:0; border-color:#e2e2e2; border-style: solid;">' . $name . $tel . $email . $agreement . $finalPrice .'</th>
				<th colspan="4" style="width: 400px; padding-top:15px; padding-bottom:15px; padding-right:15px; padding-left:15px; text-align:left; border-top:1px; border-left:1px; border-right:0; border-bottom:0; border-color:#e2e2e2; border-style: solid;">' . $region . $city . $address . $delivery . $payment . '</th>
			</tr>';
